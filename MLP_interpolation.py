...
def train_model(model, data_loader, optimizer, device, wav, YY): 
    ...
    for data in loop:
        ...
        #print(H.shape)
        loss = L_1_loss(outputs, y) + LF.L_2_loss(outputs, y) + LF.L_3_loss(outputs.reshape(dc.L,dc.J), y.reshape(dc.L,dc.J), rir, rir, wav, B_idx) + LF.L_4_loss(y.reshape(dc.L,dc.J), outputs.reshape(dc.L,dc.J), rir, wav, H, B_idx, D_idx)
        loss.backward()
        optimizer.step()
