...
def train(data, wav, epochs, model, dev):
    ...
    for epoch in range(epochs):
        ...
        for batch in loop:
            ...
            loss = L_1_loss(outputs, batch_y) + L_2_loss(pre_flat_outputs, pre_flat_batch_y) + L_3_loss(outputs, batch_y, batch_IR, batch_IR, wav, bright_batch) + L_4_loss(batch_y, batch_IR, wav, H, bright_batch, dark_batch, len(batch[2]), len(batch[3]))
            loss.backward()
            optimizer.step()
