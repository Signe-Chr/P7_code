...
def plot_mse_grid(ax, mse_grid, title):
    ...
    for x in range(mse_grid.shape[0]):
        for y in range(mse_grid.shape[1]):
            ax.text(y, x, f"{mse_grid[x, y]:.4f}", ha='center', va='center', color='w', fontsize=15)
    ...
    ax.set_xlabel("Neurons in 2nd layer", fontsize=12)
    ax.set_ylabel("Neurons in 1st layer", fontsize=12)
...
plt.tight_layout()
#plt.savefig("CV_interpolation_2_layer_plot.pdf", dpi=500)
plt.show()